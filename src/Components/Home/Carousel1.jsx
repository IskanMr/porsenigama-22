...
const Carousel1 = () => {
  ...
  const arrowStyle =
    "absolute mt-0 md:mt-7 z-10 h-10 w-10 flex items-center justify-center transform scale-35 md:scale-75 transition duration-300 md:hover:scale-100";
    ...
    >
      <span role="img" aria-label={`Arrow ${isLeft ? "left" : "right"}`}>
        <img
          src={`${
            isLeft
              ? `${process.env.PUBLIC_URL}/images/Sec1/Kiri.svg`
              : `${process.env.PUBLIC_URL}/images/Sec1/Kanan.svg`
          }`}
          alt={`${isLeft ? "Berita Kiri" : "Berita Kanan"}`}
        />
      </span>
    </button>
  );
    ...
    <div className="p-0 mt-8 md:p-12 flex justify-center w-3/4 md:w-3/4 items-center">
      <div className="relative w-full">
        <div className="carousel shadow-csel1">
          {(() => {
            if (load === "0") {
              return <div className="h-1/4x"></div>;
            } else {
              return (
                <>
                  {sliderControl(true)}
                  ...
                  <div className="w-full h-full flex justify-end absolute items-center">
                    <div
                      id="textBerita"
                      className="w-1/3 text-white text-right mr-3 md:mr-8 cursor-default flex-shrink-0"
                    >
                      ...
                      <button
                        className="text-yellow-500 cursor-pointer font-sansPro text-sm md:text-md"
                        onClick={readMore}
                      >
                        READ MORE...
                      </button>
